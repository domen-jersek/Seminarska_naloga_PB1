    ...
    try:
        ...
        if 0 <= izbira < len(racuni):
            ...
            if paket:
                ...
                print(
                    f"Transakcijski limit: {paket['osnovni_limit'] / 100 if paket['osnovni_limit'] else 'Neomejen'}"
                )
                print(
                    f"Dnevni limit: {paket['dnevni_limit'] / 100 if paket['dnevni_limit'] else 'Neomejen'}"
                )
            print(f"{'=' * 55}")
        else:
    ...
    try:
        ...
        if not transakcije:
            ...
            for tr in transakcije:
                ...
                posilja = (
                    (tr["posilja"][:34] + "..")
                    if tr["posilja"] and len(tr["posilja"]) > 36
                    else (tr["posilja"] or "-")
                )
                prejema = (
                    (tr["prejema"][:34] + "..")
                    if tr["prejema"] and len(tr["prejema"]) > 36
                    else (tr["prejema"] or "-")
                )
                znesek = tr["znesek"] / 100

                print(
                    f"{id_tr:<8} {datum:<20} {tip:<12} {posilja:<36} {prejema:<36} {znesek:>11.2f} €"
                )
...
def admin_view_accounts():
    ...
    if not racuni:
        ...
        for r in racuni:
            ...
            print(
                f"{r['IBAN']:<22} {lastnik:<25} {r['paket_tip']:<12} {stanje_eur:>11.2f} €"
            )
...
def admin_view_packages():
    ...
    if not paketi:
        ...
        for p in paketi:
            ...
            osnov = (
                f"{p['osnovni_limit'] / 100:.2f} €"
                if p["osnovni_limit"]
                else "brez"
            )
            dnevni = f"{p['dnevni_limit'] / 100:.2f} €" if p["dnevni_limit"] else "brez"
            print(
                f"{p['id_paket']:<6} {p['tip']:<14} {cena:>10} {osnov:>14} {dnevni:>14}"
            )
...
def admin_add_package():
    ...
    try:
        ...
        success, message, id_paket = bank.add_paket(
            tip, cena_cents, osnov_cents, dnevni_cents
        )
        if success:
...
def admin_edit_package():
    ...
    try:
        ...
        stari_osnov = (
            stari["osnovni_limit"] / 100 if stari["osnovni_limit"] else ""
        )
        osnov_vnos = input(
            f"Transakcijski limit v EUR [{stari_osnov if stari_osnov else 'brez'}]: "
        ).strip()
        if osnov_vnos == "":
        ...
        success, message = bank.update_paket(
            id_paket, tip, cena_cents, osnov_cents, dnevni_cents
        )
        if success:
            print(f"✅ {message}")
        else:
